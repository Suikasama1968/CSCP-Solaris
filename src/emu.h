...
class OSD {
public:
	void set_vm_node(int, const _TCHAR*) {}
};

typedef struct bitmap_s {
	inline bool initialized() { return lpBmp != NULL; }
	inline scrntype_t* get_buffer(int y) { return lpBmp + width * (height - y - 1); }
	int width, height;
	scrntype_t* lpBmp;
} bitmap_t;
...
public:
	...
	VM_TEMPLATE* get_vm() { return vm_; }
	void set_vm(VM_TEMPLATE* vm) { vm_ = vm; }
	OSD* get_osd() { return &osd_; }
...
#endif
	uint8_t* key_buffer() { return key_; }
	...
	int get_vm_window_width() { return screen_w_; }
	int get_vm_window_height() { return screen_h_; }
	scrntype_t* get_screen_buffer(int y);
#ifdef USE_SCREEN_FILTER
	void screen_skip_line(bool skip_line);
#endif
	scrntype_t* screen_buffer() { return screen_; }
